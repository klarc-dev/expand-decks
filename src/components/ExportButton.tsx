 ...
 * rebuilds the deck from the current content and produces a fresh PDF + SPA —
 * a new media record replaces `pdfFile` every run, so the export is always
 * clean (no stale PDF). Polls `lastBuildStatus` so the label tracks the build.
 */
const ExportButton: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (status === BUILD_STATUS.success) {
      ...
      toast.success('Export terminé — PDF régénéré.');
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Export lancé — reconstruction du PDF…');
  ...
  return (
    <button
      ...
      title="Reconstruit la présentation et régénère le PDF à partir du contenu actuel"
    >
      {building ? 'Export en cours…' : 'Exporter le PDF'}
    </button>
  );
};
