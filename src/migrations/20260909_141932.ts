...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_presentations_document_template" ADD VALUE 'standard-report' BEFORE 'visual-publication';`);
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_presentations_document_template" AS ENUM('presentation', 'linkedin-carousel', 'visual-publication', 'sales-sheet');
