...
**Patterns to follow:** Keep prompt prose use-case-agnostic per AGENTS.md — inject the deck's *own* values, not industry vocabulary.
...
- **importMap regeneration.** Any new admin component path (U3 if it adds a component, not just inline JSX) requires `pnpm generate:importmap` or the field silently fails to render (AGENTS.md gotcha). Prefer inline JSX in the existing `DraftFromBriefButton` to avoid this.
