...
type Fixtures = { slideCrudPresentationId: string };
...
test.describe('Payload slide-block authoring UI', () => {
  ...
  test('author adds, edits, and saves a section slide through the block editor', async ({
    ...
    const presentationId = data.slideCrudPresentationId;
    await page.goto(`/admin/collections/presentations/${presentationId}`);

    await page.getByRole('button', { name: /(?:add|ajouter) slide/i }).click();
    ...
    const firstSave = page.waitForResponse(
      (response) =>
        response.url().includes(`/api/presentations/${presentationId}`) &&
